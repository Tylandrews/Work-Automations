...
function computeMetricsForCalls(
  ...
  for (const c of calls) {
    ...
    orgSet.add(org);
    if (includePerUserCounts) perUser.set(c.user_id, (perUser.get(c.user_id) ?? 0) + 1);
...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (!url || !serviceKey) {
      return json(500, { ok: false, error: "Missing env", detail: "Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY." });
    }
    ...
    try {
      ...
      if (msg === "missing_auth" || msg === "invalid_auth") return json(401, { ok: false, error: "Unauthorized" });
      if (msg === "not_admin") return json(403, { ok: false, error: "Forbidden" });
      throw e;
    }
    ...
    return json(200, { ok: true, generated: results });
  } catch (err) {
