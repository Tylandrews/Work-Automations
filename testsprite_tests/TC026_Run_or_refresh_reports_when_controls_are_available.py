"""
TC026: Open Reports; if Refresh is usable, click it and expect either cards or an error message afterward.
"""
import asyncio
import os
...
async def run_test() -> None:
    ...
    try:
        ...
        refresh = page.locator("#reportsRefreshBtn")
        if await refresh.is_enabled():
            await refresh.click()
            await expect(page.locator("#reportsGrid")).not_to_contain_text("Loading", timeout=45000)

        err_text = (await page.locator("#reportsError").inner_text()).strip()
        body = await page.locator("#reportsModal").inner_text()
        assert err_text or "Team (weekly)" in body or "No report yet" in body or "Calls" in body

        run_btn = page.locator("#runReportsNowBtn")
        if await run_btn.is_visible() and await run_btn.is_enabled():
            await run_btn.click()
            await expect(run_btn).to_be_enabled(timeout=120000)
