"""
TC011: Log a call, move to another day with the day arrows, return, open the entry — edit modal shows details.

Creates a row with a unique caller label so the card is easy to find.
"""
import asyncio
import os
...
async def run_test() -> None:
    ...
    try:
        ...
        await page.locator("#ticketNumber").fill(e2e_notes_with_run_id(e2e_run_id, "TC011 day nav"))
        ...
        label_before = await day_label.inner_text()
        await page.locator("#historyPrevDayBtn").click()
        await expect(day_label).not_to_have_text(label_before, timeout=15000)

        label_mid = await day_label.inner_text()
        ...
        await expect(page.locator("#editMobile")).to_have_value(PHONE)
        await expect(page.locator("#editSupportRequest")).to_have_value(SUPPORT)
